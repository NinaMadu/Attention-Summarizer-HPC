...
static void print_resource_report(double functional_total_sec) {
    printf("\n");
    print_separator('=', 70);
    printf("  RESOURCE USAGE REPORT  [CUDA + OPENMP HYBRID]\n");
    print_separator('=', 70);
#ifndef _WIN32
    struct rusage usage;
...
static void print_report(double total_start) {
    ...
    printf("\n");
    print_separator('=', 70);
    printf("  COMPLEXITY & TIMING REPORT  [CUDA + OPENMP HYBRID]\n");
    ...
    print_separator('=', 70);
    printf("\n  Complexity summary:\n");
    printf("    load_glove           O(V x D)          V=%d, D=%d  [Serial I/O]\n", MAX_VOCAB, EMBED_DIM);
    ...
    printf("    vectorize            O(T x V)          token lookup  [OpenMP]\n");
    printf("    positional_encoding  O(T x D)          [OpenMP]\n");
    ...
    printf("    sentence_scoring     O(T^2 + T)        [OpenMP]\n");
    print_separator('=', 70);
    printf("\n");
...
void compute_pca_projections_cuda(double total_start) {
    ...
    /* Extract float vectors from AoS vocab[] into flat GPU upload array */
    float *flat_vecs = (float *)malloc((size_t)vocab_size * EMBED_DIM * sizeof(float));
    if (!flat_vecs) { printf("ERROR: flat_vecs alloc failed\n"); return; }
    printf("  [cov] Flattening vocabulary on CPU with OpenMP (%d threads max)...\n",
           omp_get_max_threads());
    ...
    #pragma omp parallel for collapse(2) reduction(+:w_ops) schedule(static)
...
void vectorize(char tokens[MAX_TOKENS][MAX_WORD_LEN], int n_tokens,
    ...
    int hit = 0;
    long long cmp_ops = 0;
    int *found_flags = (int *)calloc((size_t)n_tokens, sizeof(int));
    if (!found_flags) {
        printf("ERROR: found_flags alloc failed\n");
        return;
    }

    printf("  [vec] OpenMP lookup using up to %d threads\n", omp_get_max_threads());
...
void add_positional_encoding(float embeddings[MAX_TOKENS][EMBED_DIM], int n_tokens) {
    ...
    double t0 = now_sec();
    long long ops = 0;
    printf("  [pe] OpenMP positional encoding using up to %d threads\n", omp_get_max_threads());
    #pragma omp parallel for schedule(static) reduction(+:ops)
...
void summarize(char tokens[MAX_TOKENS][MAX_WORD_LEN],
    ...
    printf("  [sum] OpenMP token centrality using up to %d threads\n", omp_get_max_threads());
    #pragma omp parallel for schedule(static)
    for (int j = 0; j < n_tokens; j++) {
        float score = 0.0f;
        for (int i = 0; i < n_tokens; i++)
            score += attn_weights[i][j];
        metrics->token_score[j] = score / ((n_tokens > 0) ? n_tokens : 1);
    }
...
int main() {
    ...
    printf("\n");
    print_separator('=', 70);
    printf("  Attention Summarizer: GloVe 300D + PCA Whitening  [CUDA + OPENMP HYBRID]\n");
