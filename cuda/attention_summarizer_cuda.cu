...
/* Suppress MSVC warnings about "unsafe" CRT functions like fopen, strcpy. */
#ifdef _MSC_VER
...
/* Windows vs POSIX high-resolution timer headers. */
#ifdef _WIN32
#include <windows.h>
#else
#include <time.h>
...
/* Maximum tokens the model can process per input paragraph. */
#define MAX_TOKENS       1024

/* Maximum byte length of a single token (word). */
#define MAX_WORD_LEN     64

/* Maximum number of sentences detected in the input. */
#define MAX_SENTENCES    128

/* Maximum character length of a single extracted sentence. */
#define MAX_SENTENCE_LEN 16384

/* Maximum input paragraph size in bytes. */
#define MAX_PARAGRAPH_CHARS 65536

/* Maximum output summary buffer size. */
#define MAX_SUMMARY_CHARS   65536

/* GloVe embedding dimensionality */
#define EMBED_DIM        300

/* Maximum number of vocabulary entries loaded from GloVe. */
#define MAX_VOCAB        500000

/* Number of attention heads (must divide EMBED_DIM evenly). */
#define NUM_HEADS        12

/* Per-head embedding slice width: 300 / 12 = 25. */
#define HEAD_DIM         (EMBED_DIM / NUM_HEADS)

/* Jacobi sweeps over the full matrix before stopping early. */
#define JACOBI_SWEEPS    100

/* Eigenvalue threshold — values below this are treated as zero variance. */
...
/* Number of rows each thread block covers in the transpose tile. */
 ...
 * On failure it prints the file, line, and error string, then exits.
 */
...
#ifdef _WIN32
/* Windows: use QueryPerformanceCounter for sub-millisecond resolution. */
...
#else
/* POSIX: use CLOCK_MONOTONIC (unaffected by wall-clock adjustments). */
static double now_sec() {
    struct timespec ts;
...
/* One record per pipeline stage: name, wall time, and operation count. */
typedef struct {
    const char *name;
    double      elapsed_sec;
    long long   ops;
...
#define MAX_STAGES 16                   /* upper bound on logged stages */
static StageLog stage_log[MAX_STAGES]; /* ring of stage records        */
static int      stage_count = 0;       /* how many stages recorded     */

/* Append a completed stage to the log. Silently drops if log is full. */
...
/* Print a horizontal rule made of character c, width columns wide. */
...
/*
 * Print the final timing & complexity report.
 * total_start is the wall-clock second at which the program began
 * (adjusted to exclude stdin wait time).
 */
static void print_report(double total_start) {
    ...
    for (int i = 0; i < stage_count; i++) {
        ...
        int    bar_len = (int)(pct / 5.0); /* each '#' = 5 % of total time */
        char   bar[22];
    ...
    /* Big-O summary for each major stage. */
    printf("\n  Complexity summary:\n");
...
/* When non-zero, Jacobi sweep progress is printed every 10 sweeps. */
static int jacobi_verbose = 1;

/* ====================== STRUCTS ====================== */

/* One entry in the GloVe vocabulary: a word string and its 300-D vector. */
typedef struct {
...
/*
 * Intermediate & final scoring data produced during summarization.
 * Passed between summarize() and print_comparison_report().
 */
typedef struct {
    float token_score[MAX_TOKENS];    /* average incoming attention per token    */
    float sent_score[MAX_SENTENCES];  /* average token score per sentence        */
    int   sent_count[MAX_SENTENCES];  /* tokens assigned to each sentence        */
    int   selected[2];                /* indices of the two best sentences       */
} SummaryMetrics;

/* ====================== GLOBALS ====================== */

/* Heap-allocated GloVe vocabulary (up to MAX_VOCAB entries). */
Embedding *vocab;
int        vocab_size = 0;

/*
 * Query and Key projection matrices built from PCA whitening.
 * Both are set to the same whitened eigenvector matrix so the
 * attention geometry is isotropic in PCA space.
 */
...
/*
 * K1: compute_mean_kernel
 *
 * Computes the per-dimension mean over V vocabulary vectors stored in
 * column-major layout d_vecs[d][v] (dimension-major, also called dim-major).
 *
 * Grid:  EMBED_DIM blocks  — one block per output dimension d.
 * Block: BLOCK_SIZE threads — cooperatively reduce V values with shared-mem tree.
 *
 * Each thread accumulates a partial sum over its stride of vocabulary indices,
 * then the block reduces via a binary tree in shared memory, and thread 0
 * stores mean[d] = total_sum / V.
 */
__global__ void compute_mean_kernel(const float *vecs, float *mean, int V) {
    int d = blockIdx.x;            /* which dimension this block computes  */
    if (d >= EMBED_DIM) return;

    __shared__ float sdata[BLOCK_SIZE]; /* per-thread partial sums */
    int   tid       = threadIdx.x;
    float local_sum = 0.0f;

    /* Each thread strides over vocabulary columns at row d. */
    ...
    /* Standard parallel reduction: halve active threads each step. */
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
        if (tid < s) sdata[tid] += sdata[tid + s];
        __syncthreads();
    }

    /* Thread 0 writes the final mean value. */
    if (tid == 0) mean[d] = sdata[0] / V;
}

/*
 * K2: covariance_kernel
 *
 * Computes one element of the D×D covariance matrix per block, covering
 * only the lower triangle (j ≤ i) and then mirroring symmetrically.
 *
 * pair_idx is a linear index into the lower-triangular pair set:
 *   pair_idx = i*(i+1)/2 + j
 * We invert this to find (i, j) via the quadratic formula.
 *
 * Grid:  D*(D+1)/2 blocks  — one block per unique (i,j) pair.
 * Block: BLOCK_SIZE threads — reduce over V vocabulary items.
 *
 * cov[i][j] = cov[j][i] = Σ_v (vec[i][v]-mean[i])*(vec[j][v]-mean[j]) / (V-1)
 */
__global__ void covariance_kernel(const float *vecs, const float *mean,
                                   float *cov, int V) {
    int pair_idx = (int)blockIdx.x;

    /* Recover row index i from the triangular pair index. */
    int i         = (int)((sqrtf(8.0f * (float)pair_idx + 1.0f) - 1.0f) * 0.5f);
    int row_start = i * (i + 1) / 2;
    if (row_start > pair_idx) {   /* clamp rounding errors from sqrtf */
        i--;
        row_start = i * (i + 1) / 2;
    }
    int j = pair_idx - row_start; /* column index within row i */
    ...
    /* Accumulate cross-product of centered values over vocabulary. */
    for (int v = tid; v < V; v += blockDim.x)
    ...
    /* Thread 0 stores the sample covariance and mirrors the off-diagonal. */
    if (tid == 0) {
        float val          = sdata[0] / (V - 1);
        cov[i * EMBED_DIM + j] = val;
        cov[j * EMBED_DIM + i] = val; /* symmetry */
    }
}

/*
 * K3: transpose_vocab_kernel
 *
 * Converts the vocabulary matrix from row-major [V][D] (one word per row)
 * to dimension-major [D][V] (one dimension per row) using a shared-memory
 * tiled transpose to avoid uncoalesced global memory accesses.
 *
 * The covariance and mean kernels then read contiguous memory for each
 * dimension, improving cache and memory-bandwidth efficiency.
 *
 * Grid:  (ceil(D/TILE), ceil(V/TILE))  — tiles covering the D×V matrix.
 * Block: (TILE_DIM, BLOCK_ROWS)        — each thread handles TILE/BLOCK_ROWS rows.
 */
__global__ void transpose_vocab_kernel(const float *row_major, float *dim_major, int V) {
    /* +1 in shared-mem column avoids bank conflicts during the transposed write. */
    __shared__ float tile[TRANSPOSE_TILE_DIM][TRANSPOSE_TILE_DIM + 1];

    int in_d = blockIdx.x * TRANSPOSE_TILE_DIM + threadIdx.x; /* source dimension */
    int in_v = blockIdx.y * TRANSPOSE_TILE_DIM + threadIdx.y; /* source vocab row  */

    /* Load a TILE×TILE region from row-major source into shared memory. */
    ...
    /* Write the tile to the transposed destination in dim-major layout. */
 ...
 * K4: project_kernel
 *
 * Multiplies the embedding matrix by a weight matrix:
 *   out[i] = in[i] × W      (EMBED_DIM → EMBED_DIM, one token per block)
 *
 * Used for both Q and K projections during multi-head attention.
 *
 * Grid:  n_tokens blocks — one block per input token.
 * Block: BLOCK_SIZE threads — each thread computes one or more output dims.
 */
__global__ void project_kernel(const float *in, const float *W, float *out,
    ...
    /* Each thread strides over output dimensions. */
 ...
 *   scores[h][i][j] = (Q[i, head_h] · K[j, head_h]) * scale
 *
 * where scale = 1/sqrt(HEAD_DIM) to prevent gradient vanishing in softmax.
 *
 * Grid:  (ceil(T/BLOCK), T, H)  — j across x, i across y, head across z.
 * Block: BLOCK_SIZE threads     — each computes one (i,j) score per head.
 */
__global__ void qk_dot_kernel(const float *Q, const float *K, float *scores,
                               int n_tokens, float scale) {
    int j = blockIdx.x * blockDim.x + threadIdx.x; /* key token index   */
    int i = blockIdx.y;                             /* query token index */
    int h = blockIdx.z;                             /* attention head    */

    if (i >= n_tokens || j >= n_tokens) return;

    int   offset = h * HEAD_DIM; /* start of this head's slice in the embedding */
    float dot    = 0.0f;

    for (int d = 0; d < HEAD_DIM; d++)
        dot += Q[i * EMBED_DIM + offset + d] * K[j * EMBED_DIM + offset + d];

    /* Write scaled score into the flat [H, T, T] scores tensor. */
 ...
 * K6: softmax_kernel
 *
 * Row-wise numerically stable softmax: subtract the row maximum before
 * computing exp() to prevent overflow.
 *
 * Each thread handles one (head h, query i) row of length n_tokens.
 *
 * Grid:  ceil(H*T / BLOCK)  — flattened over all head-query pairs.
 * Block: BLOCK_SIZE threads.
 */
__global__ void softmax_kernel(float *scores, int n_heads, int n_tokens) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int h   = idx / n_tokens; /* head index  */
    int i   = idx % n_tokens; /* query index */
    ...
    /* Pass 2: exponentiate and accumulate normalizer. */
    float sum = 0.0f;
    ...
    /* Pass 3: normalize so the row sums to 1. */
    for (int j = 0; j < n_tokens; j++)
        row[j] /= sum;
}

/*
 * K7: attn_output_kernel
 *
 * Computes the attention-weighted sum of value vectors:
 *   out[i][offset+dd] = Σ_j  attn[h][i][j] × V_mat[j][offset+dd]
 *
 * Here V_mat is the original embedding matrix (no separate projection).
 *
 * Grid:  (ceil(HEAD_DIM/32), T, H)  — dd across x, token across y, head z.
 * Block: 32 threads.
 */
__global__ void attn_output_kernel(const float *attn, const float *V_mat,
                                    float *out, int n_tokens) {
    int dd = blockIdx.x * blockDim.x + threadIdx.x; /* dim within the head slice */
    int i  = blockIdx.y;                             /* output token index        */
    int h  = blockIdx.z;                             /* attention head            */
    ...
    /* Weighted sum over all key/value tokens. */
    for (int j = 0; j < n_tokens; j++)
...
/*
 * K8: average_heads_kernel
 *
 * Averages attention weights across all NUM_HEADS heads to produce a
 * single T×T attention matrix used for downstream sentence scoring.
 *
 * Grid:  (ceil(T/BLOCK), T)  — j across x, i across y.
 * Block: BLOCK_SIZE threads.
 */
__global__ void average_heads_kernel(const float *head_attn, float *avg_attn,
                                      int n_tokens) {
    int j = blockIdx.x * blockDim.x + threadIdx.x; /* column (key token)   */
    int i = blockIdx.y;                             /* row    (query token) */
...
/*
 * K9: head_diag_mean_kernel
 *
 * Computes the mean diagonal attention weight for each head
 * (i.e., how strongly each token attends to itself).
 * Used only for the diagnostic print; avoids copying the full scores tensor.
 *
 * Grid:  NUM_HEADS blocks — one block per head.
 * Block: BLOCK_SIZE threads — parallel reduction over token diagonal.
 */
__global__ void head_diag_mean_kernel(const float *head_attn, float *diag_mean,
                                      int n_tokens) {
    int h   = blockIdx.x;   /* attention head */
    int tid = threadIdx.x;
    ...
    /* Each thread accumulates diagonal elements over its stride. */
    ...
    /* Thread 0 stores mean self-attention weight for this head. */
    if (tid == 0) diag_mean[h] = sdata[0] / n_tokens;
}

/* ===========================================================================
 *                     CPU FUNCTIONS  (identical to serial baseline)
 * =========================================================================== */
 ...
 * jacobi_eigen
 *
 * Classic Jacobi iteration: repeatedly zeroes off-diagonal elements of a
 * real symmetric matrix A by applying plane (Givens) rotations, converging
 * to a diagonal matrix whose diagonal is the set of eigenvalues.
 *
 * A         [in]  Symmetric D×D matrix (covariance); untouched — a copy S is made.
 * V         [out] Orthonormal eigenvector matrix (columns are eigenvectors).
 * d         [out] Eigenvalues (diagonal of S after convergence).
 * op_count  [out] Total floating-point multiplications performed.
 *
 * Complexity: O(JACOBI_SWEEPS × D²) rotation ops, each O(D) → O(D³) total.
 */
void jacobi_eigen(float A[EMBED_DIM][EMBED_DIM],
    ...
    *op_count = 0;
    ...
    /* V starts as the identity; each rotation updates it to track eigenvectors. */
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            V[i][j] = (i == j) ? 1.0f : 0.0f;

    /* S is a working copy of A that gets diagonalized in-place. */
    ...
    for (int sweep = 0; sweep < JACOBI_SWEEPS; sweep++) {
        /* Sum squared off-diagonal entries to measure convergence. */
        float off = 0.0f;
        for (int i = 0; i < n; i++)
            for (int j = i + 1; j < n; j++)
                off += S[i][j] * S[i][j];

        /* Periodically report progress if verbose mode is on. */
        ...
        /* Early exit when the matrix is essentially diagonal. */
        if (off < 1e-10f) {
            printf("  [jacobi] Converged at sweep %d (off < 1e-10)\n", sweep);
            break;
        }

        /* One full sweep: zero every off-diagonal pair (p, q). */
        for (int p = 0; p < n - 1; p++) {
            for (int q = p + 1; q < n; q++) {
                if (fabsf(S[p][q]) < 1e-12f) continue; /* already tiny, skip */

                /* Compute the Givens rotation angle θ = 0.5 * atan2(2S_pq, S_pp - S_qq). */
                float theta   = 0.5f * atan2f(2.0f * S[p][q], S[p][p] - S[q][q]);
                float c       = cosf(theta);
                float s_angle = sinf(theta);

                /* Update the two diagonal entries using the rotation. */
                float new_Spp = c*c*S[p][p] + 2*s_angle*c*S[p][q] + s_angle*s_angle*S[q][q];
                float new_Sqq = s_angle*s_angle*S[p][p] - 2*s_angle*c*S[p][q] + c*c*S[q][q];

                /* Apply the rotation to all other rows/columns of S. */
                for (int r = 0; r < n; r++) {
                    if (r == p || r == q) continue;
                    float new_rp =  c * S[r][p] + s_angle * S[r][q];
                    float new_rq = -s_angle * S[r][p] + c * S[r][q];
                    S[r][p] = S[p][r] = new_rp;
                    S[r][q] = S[q][r] = new_rq;
                    *op_count += 4;
                }
                ...
                S[p][p] = new_Spp;
                S[q][q] = new_Sqq;
                S[p][q] = S[q][p] = 0.0f; /* zeroed by construction */

                /* Apply the same rotation to accumulate eigenvectors in V. */
                for (int r = 0; r < n; r++) {
                    float new_rp =  c * V[r][p] + s_angle * V[r][q];
                    float new_rq = -s_angle * V[r][p] + c * V[r][q];
                    V[r][p] = new_rp;
                    V[r][q] = new_rq;
                    *op_count += 4;
                }
    ...
    /* The diagonal of the converged S holds the eigenvalues. */
    for (int i = 0; i < n; i++)
        d[i] = S[i][i];
...
/*
 * compute_pca_projections_cuda
 *
 * Orchestrates three sub-stages on the loaded GloVe vocabulary:
 *   1. GPU  — transpose vocab, compute per-dim mean, compute covariance matrix.
 *   2. CPU  — Jacobi eigendecomposition on the D×D covariance matrix.
 *   3. CPU  — Build whitening matrices W_Q = W_K = V × diag(λ^-0.5).
 *
 * total_start is passed in only for the outer timer (not used here directly).
 */
void compute_pca_projections_cuda(double total_start) {
    (void)total_start; /* parameter reserved for potential future use */

    /* ---- Stage 1: covariance_matrix [GPU] ---- */
    printf("\n");
    print_separator('-', 60);
    ...
    /* Flatten AoS vocab[] into a contiguous row-major [V][D] float array.
     * Try pinned (page-locked) memory first for faster DMA transfers. */
    ...
    if (host_err != cudaSuccess) {
        ...
        printf("  [cov] Pinned allocation unavailable (%s); using pageable memory.\n",
               cudaGetErrorString(host_err));
        cudaGetLastError(); /* clear the non-fatal error from the driver */
    }

    /* Copy each word's 300-D vector into the flat buffer (row per word). */
    for (int v = 0; v < vocab_size; v++)
        memcpy(flat_vecs + (size_t)v * EMBED_DIM, vocab[v].vec, EMBED_DIM * sizeof(float));

    /* Allocate GPU buffers: row-major source, dim-major transposed, mean, covariance. */
    ...
    /* Upload row-major vocab to GPU, then free the host staging buffer. */
    CUDA_CHECK(cudaMemcpy(d_vecs_row, flat_vecs, vecs_bytes, cudaMemcpyHostToDevice));
    if (flat_vecs_pinned) cudaFreeHost(flat_vecs);
    else                  free(flat_vecs);

    /* Transpose on GPU so mean/covariance kernels read contiguous data per dim. */
    ...
    cudaFree(d_vecs_row); /* no longer needed after transposition */
    ...
    /* One block per unique lower-triangular (i,j) pair. */
    int  cov_pairs = EMBED_DIM * (EMBED_DIM + 1) / 2;
    dim3 cov_grid(cov_pairs);
    printf("  [cov] Computing %dx%d covariance on GPU (%d blocks x %d threads)...\n",
           EMBED_DIM, EMBED_DIM, cov_pairs, BLOCK_SIZE);
    covariance_kernel<<<cov_grid, BLOCK_SIZE>>>(d_vecs, d_mean, d_cov, vocab_size);
    CUDA_CHECK(cudaPeekAtLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    /* Download covariance to CPU for Jacobi (which must run on CPU). */
    float *cov_flat       = NULL;
    ...
    if (host_err != cudaSuccess) {
        ...
        if (!cov_flat) {
            ...
            cudaFree(d_vecs); cudaFree(d_mean); cudaFree(d_cov);
            return;
        }
        printf("  [cov] Pinned download allocation unavailable (%s); using pageable.\n",
               cudaGetErrorString(host_err));
        cudaGetLastError();
    }

    CUDA_CHECK(cudaMemcpy(cov_flat, d_cov,
                           EMBED_DIM * EMBED_DIM * sizeof(float),
                           cudaMemcpyDeviceToHost));

    /* Free the large (~400 MB) GPU vocab buffers immediately. */
    cudaFree(d_vecs);
    cudaFree(d_mean);
    cudaFree(d_cov);
    ...
    /* ---- Stage 3: build_whitening_matrices [CPU] ---- */
    printf("\n");
    print_separator('-', 60);
    ...
    /* Show the top-5 eigenvalues so the user can gauge explained variance. */
    ...
    /* Whitening: W[i][j] = eigvec[i][j] / sqrt(eigval[j])
     * Eigenvalues below EIG_FLOOR are clamped to avoid division by ~0. */
    for (int i = 0; i < EMBED_DIM; i++) {
        for (int j = 0; j < EMBED_DIM; j++) {
            float scale = (eigvals[j] > EIG_FLOOR) ? 1.0f / sqrtf(eigvals[j]) : 0.0f;
            W_Q[i][j]   = eigvecs[i][j] * scale;
            W_K[i][j]   = eigvecs[i][j] * scale; /* identical for symmetric attention */
            w_ops++;
        }
    }
 ...
 * load_glove
 *
 * Reads a GloVe text file line by line.
 * Each line format: "<word> <f1> <f2> ... <fD>\n"
 *
 * Allocates the global vocab[] array (up to MAX_VOCAB entries).
 * Returns 1 on success, 0 on failure.
 *
 * Complexity: O(V × D) — one float parse per (word, dimension) pair.
 */
int load_glove(const char *filename) {
    ...
    while (fgets(line, sizeof(line), f) && vocab_size < MAX_VOCAB) {
        /* First token on each line is the word string. */
        ...
        /* Remaining tokens are floating-point embedding dimensions. */
...
/*
 * is_token_delim: returns true for characters that end or separate tokens.
 * Space, punctuation, brackets, quotes, and null all delimit tokens.
 */
...
/*
 * is_sentence_delim: returns true for characters that end a sentence.
 * We treat '.', '!', and '?' as sentence terminators.
 */
static int is_sentence_delim(char c) {
    return c == '.' || c == '!' || c == '?';
}

/* Strip leading and trailing whitespace from a C string in-place. */
static void trim_sentence(char *s) {
    /* Move start past leading whitespace. */
    int start = 0;
 ...
 * tokenize
 *
 * Splits 'text' into lowercase tokens and sentences simultaneously.
 *
 * tokens         [out] Array of extracted lowercase token strings.
 * token_sentence [out] Sentence index each token belongs to.
 * sentences      [out] Raw sentence text (trimmed, without the delimiter).
 * n_sentences    [out] Number of sentences detected.
 *
 * Returns the total number of tokens extracted (≤ MAX_TOKENS).
 */
int tokenize(char *text,
    ...
    int  sent_len = 0;    /* characters accumulated in current sentence */
    int  tok_len  = 0;    /* characters accumulated in current token    */
    ...
    for (int i = 0; ; i++) {
        ...
        /* Append character to the running sentence buffer
         * (stop before sentence delimiters and beyond-bounds). */
        ...
        /* Accumulate non-delimiter characters into the token buffer (lowercase). */
        if (!is_token_delim(c)) {
            if (tok_len < MAX_WORD_LEN - 1)
                token_buf[tok_len++] = (char)tolower((unsigned char)c);
        } else if (tok_len > 0 && n < MAX_TOKENS) {
            /* Delimiter hit with a pending token — store it. */
            token_buf[tok_len] = '\0';
            strncpy(tokens[n], token_buf, MAX_WORD_LEN - 1);
            tokens[n][MAX_WORD_LEN - 1] = '\0';
            token_sentence[n]            = s; /* associate token with its sentence */
            n++;
            tok_len = 0;
        } else {
            tok_len = 0; /* reset partial token on repeated delimiter */
        }

        /* On sentence-ending punctuation or null, finalize the current sentence. */
        if (is_sentence_delim(c) || c == '\0') {
            if (s < MAX_SENTENCES && sent_len > 0) {
                trim_sentence(sentences[s]);
                if (strlen(sentences[s]) > 0) s++; /* advance only if non-empty */
            }
            sent_len = 0;
 ...
 * vectorize
 *
 * Maps each token string to its GloVe embedding vector.
 * OOV (out-of-vocabulary) tokens get a zero vector.
 *
 * Complexity: O(T × V) — linear scan of the vocab per token.
 * (A hash map would reduce this to O(T), but GloVe loading is the bottleneck.)
 */
void vectorize(char tokens[MAX_TOKENS][MAX_WORD_LEN], int n_tokens,
    ...
    int       hit     = 0;      /* tokens successfully matched in vocab */
    long long cmp_ops = 0;      /* total string comparisons performed   */
    ...
    for (int i = 0; i < n_tokens; i++) {
        ...
        if (!found) {
            /* Zero vector signals OOV — still participates in attention. */
 ...
 *
 * Adds the standard sinusoidal positional encoding from "Attention Is All You Need":
 *   PE[pos][2k]   = sin(pos / 10000^(2k/D))
 *   PE[pos][2k+1] = cos(pos / 10000^(2k/D))
 *
 * This injects word-order information into otherwise order-invariant embeddings.
 * Complexity: O(T × D).
 */
void add_positional_encoding(float embeddings[MAX_TOKENS][EMBED_DIM], int n_tokens) {
    ...
    for (int pos = 0; pos < n_tokens; pos++) {
        for (int dd = 0; dd < EMBED_DIM; dd++) {
            /* Compute the denominator base (10000^(2i/D)). */
...
/*
 * multihead_self_attention_cuda
 *
 * Runs the full multi-head self-attention pipeline on the GPU:
 *   1. Project embeddings → Q and K via W_Q, W_K  (project_kernel)
 *   2. Scaled dot-product QK^T for every head      (qk_dot_kernel)
 *   3. Row-wise stable softmax                      (softmax_kernel)
 *   4. Attention-weighted sum of V (= embeddings)  (attn_output_kernel)
 *   5. Average attention maps across heads          (average_heads_kernel)
 *   6. Per-head diagonal mean for diagnostics       (head_diag_mean_kernel)
 *
 * Results:
 *   attn_weights [out]  Averaged T×T attention map (used for sentence scoring).
 *   output       [out]  Context-enriched token embeddings (T × EMBED_DIM).
 */
void multihead_self_attention_cuda(float embeddings[MAX_TOKENS][EMBED_DIM],
    ...
    /* Zero output buffers to avoid accumulation of stale values. */
    ...
    /* Upload embeddings and weight matrices to GPU. */
    ...
    /* ---- Scaled QK^T ---- */
    float scale = 1.0f / sqrtf((float)HEAD_DIM); /* stabilises softmax gradients */
    ...
    /* ---- Row-wise softmax over all (head, query) pairs ---- */
    ...
    head_diag_mean_kernel<<<NUM_HEADS, BLOCK_SIZE>>>(d_head_scores,
                                                       d_diag_mean, n_tokens);
    CUDA_CHECK(cudaPeekAtLastError());
    ...
    /* ---- Attention-weighted output (V = original embeddings) ---- */
    ...
    long long total_ops  = proj_ops + qk_ops + out_ops;
    log_stage("self_attention", t0, t1, total_ops);
    printf("  [attn] Total ops — project:%lld  QKt:%lld  output:%lld  sum:%lld\n",
           proj_ops, qk_ops, out_ops, total_ops);
    printf("  [attn] Attention stage finished in %.4fs\n", t1 - t0);

    /* Free all GPU allocations for this stage. */
 ...
 * token_importance
 *
 * Returns the L2 norm (Euclidean length) of a token's attention output vector.
 * Tokens with larger norms received more concentrated attention signal.
 */
 ...
 * summarize
 *
 * Scores sentences by the average incoming attention centrality of their tokens,
 * then selects the top-2 sentences to form the summary.
 *
 * Algorithm:
 *   1. token_score[j]  = mean over all i of attn_weights[i][j]
 *      (how much, on average, other tokens attend to token j)
 *   2. sent_score[s]   = mean of token_score over tokens in sentence s
 *   3. Select the two sentences with the highest sent_score.
 *   4. Output them in original document order.
 */
void summarize(char  tokens[MAX_TOKENS][MAX_WORD_LEN],
    ...
    /* Suppress "unused parameter" warnings for tokens/output (reserved for future use). */
    (void)tokens;
    (void)output;
    ...
    /* Step 1: compute column-sum of attention matrix, normalised by T. */
    for (int j = 0; j < n_tokens; j++) {
        for (int i = 0; i < n_tokens; i++)
    ...
    /* Step 2: accumulate token scores per sentence. */
    for (int i = 0; i < n_tokens; i++) {
    ...
    /* Step 3: normalize to per-token average, then display. */
    ...
    /* Step 4: find top-2 scoring sentences (best, second-best). */
    int   best[2]   = {-1,    -1};
    float bscore[2] = {-1.0f, -1.0f};
    for (int s = 0; s < n_sent; s++) {
        if      (metrics->sent_score[s] > bscore[0]) {
            bscore[1] = bscore[0]; best[1] = best[0];
            bscore[0] = metrics->sent_score[s]; best[0] = s;
        }
        else if (metrics->sent_score[s] > bscore[1]) {
            bscore[1] = metrics->sent_score[s]; best[1] = s;
        }
    }
    ...
    /* Step 5: emit sentences in their original order for readability. */
    int order[2];
    ...
    for (int k = 0; k < 2; k++)
        if (order[k] != -1) {
            strcat(summary, sentences[order[k]]);
            strcat(summary, ".\n");
        }
    strcat(summary, "======================================================\n");
}
 ...
 *
 * Emits a set of COMPARE_* lines that can be diffed against other
 * implementations (serial, MPI, OpenMP) to verify numerical correctness.
 *
 * Checksums use position-weighted sums to catch element-ordering bugs.
 */
void print_comparison_report(char  tokens[MAX_TOKENS][MAX_WORD_LEN],
                              ...
                              float output[MAX_TOKENS][EMBED_DIM],
                              SummaryMetrics *metrics) {
    /* Accumulators for weighted checksums and range statistics. */
    ...
    /* Compute attention matrix statistics. */
    for (int i = 0; i < n_tokens; i++) {
        double row_sum = 0.0;
        for (int j = 0; j < n_tokens; j++) {
            float  v      = attn_weights[i][j];
            double weight = (double)(i + 1) * (double)(j + 1); /* position weight */
    ...
    /* Compute output embedding statistics. */
    for (int i = 0; i < n_tokens; i++) {
        for (int dd = 0; dd < EMBED_DIM; dd++) {
            float  v      = output[i][dd];
    ...
    /* Guard against zero-token edge case. */
    if (n_tokens == 0) {
        row_sum_min = 0.0;
        row_sum_max = 0.0;
        attn_min    = 0.0f;
    ...
    /* Report the top-2 selected sentences with their scores. */
    ...
    printf("COMPARE_ATTENTION,checksum,%.9e,abs_checksum,%.9e,"
           "min,%.9e,max,%.9e,row_sum_min,%.9e,row_sum_max,%.9e\n",
           attn_checksum, attn_abs_checksum,
           (double)attn_min, (double)attn_max,
           row_sum_min, row_sum_max);
    printf("COMPARE_OUTPUT,checksum,%.9e,abs_checksum,%.9e\n",
           out_checksum, out_abs_checksum);

    /* Per-sentence table for structured comparison. */
    printf("\nCOMPARE_SENTENCE_TABLE\n");
    printf("sentence_id,token_count,score\n");
    for (int s = 0; s < n_sent; s++)
        printf("%d,%d,%.9f\n", s, metrics->sent_count[s], metrics->sent_score[s]);

    /* Per-token table with attention centrality and output norm. */
    printf("\nCOMPARE_TOKEN_TABLE\n");
    ...
    /* Sparse sample of the attention matrix (8×8 evenly-spaced indices). */
    int sample = (n_tokens < 8) ? n_tokens : 8;
    printf("\nCOMPARE_ATTENTION_SAMPLE_%dx%d\n", sample, sample);
    for (int si = 0; si < sample; si++) {
        /* Map sample index to actual token index (evenly spaced). */
        int i = (sample <= 1) ? 0
                              : (int)((long long)si * (n_tokens - 1) / (sample - 1));
        for (int sj = 0; sj < sample; sj++) {
            int j = (sample <= 1) ? 0
                                  : (int)((long long)sj * (n_tokens - 1) / (sample - 1));
...
/* Common English function words that carry little semantic content. */
...
/* Returns 1 if word w is in the stopword list, 0 otherwise. */
 ...
 * print_top_words
 *
 * Ranks content words (non-stopwords, length ≥ 2) by their attention output
 * L2 norm, deduplicating repeated tokens, and displays the top-10 with a
...
#define TOP_N 10
    ...
    for (int i = 0; i < n_tokens; i++) {
        if (is_stopword(tokens[i]))        continue; /* skip function words */
        if (strlen(tokens[i]) < 2)         continue; /* skip single-char tokens */

        float score = token_importance(output[i]);

        /* If this word appeared before, update its score to the max seen. */
        int dup = 0;
    ...
    /* Partial selection sort: pull the TOP_N highest scores to the front. */
    ...
    float max_score = (take > 0) ? ws[0].score : 1.0f; /* for bar scaling */
    ...
    for (int i = 0; i < take; i++) {
        int  bar_len = (int)(20.0f * ws[i].score / max_score); /* proportional bar */
        char bar[22];
        for (int b = 0; b < 20; b++) bar[b] = (b < bar_len) ? '*' : ' ';
        bar[20] = '\0';
...
int main() {
    /* ---- Stack / heap allocations for the full pipeline ---- */
    ...
    /* Large arrays on the heap to avoid stack overflow. */
    float (*embeddings)[EMBED_DIM]    = (float (*)[EMBED_DIM])
                                         malloc(MAX_TOKENS * sizeof(*embeddings));
    float (*attn_weights)[MAX_TOKENS] = (float (*)[MAX_TOKENS])
                                         malloc(MAX_TOKENS * sizeof(*attn_weights));
    float (*output)[EMBED_DIM]        = (float (*)[EMBED_DIM])
                                         malloc(MAX_TOKENS * sizeof(*output));
    SummaryMetrics metrics;
    ...
    /* Print GPU properties for transparency and reproducibility. */
    cudaDeviceProp prop;
    ...
    /* ---- Read user paragraph (exclude stdin wait from timing) ---- */
    ...
    /* Add back the time spent waiting for I/O so it doesn't distort stage %s. */
    double io_wait_time = now_sec() - io_wait_start;
    total_start += io_wait_time;

    /* ---- NLP pipeline ---- */
    int n_tokens = tokenize(paragraph, tokens, token_sentence,
